...
const findByCourse = async (courseId) => {
  const [rows] = await db.query(`SELECT * FROM assignments WHERE course_id = ? ORDER BY id DESC`, [courseId]);
  return rows;
};
...
const createAssignment = async (data) => {
  const [result] = await db.query(
    ...
    [data.course_id, data.title, data.description || null, data.due_date || null, data.max_score || 10]
  );
